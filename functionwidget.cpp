...
void FunctionWidget::set_function(int f, std::string name)
{
  ...
  QModelIndex index = functions_model->index(f, 2, QModelIndex());
...
void FunctionWidget::set_function_name(int f, std::string name)
{
  if(f>n_funcs-1)
  {
    set_n_functions(f+1);
...
std::string FunctionWidget::get_function_name(int f)
{
  if(f>n_funcs-1)
  {
    return std::string("");
...
void FunctionWidget::set_function_active(int f, bool active)
{
  if(f>n_funcs-1)
  {
    set_n_functions(f+1);
  }
  QModelIndex index = functions_model->index(f, 1, QModelIndex());
  if(active)
  {
    functions_model->setData(index, QVariant(Qt::Checked), Qt::CheckStateRole);
  }
  else
  {
    functions_model->setData(index, QVariant(Qt::Unchecked), Qt::CheckStateRole);
  }
}

bool FunctionWidget::get_function_active(int f)
{
  if(f>n_funcs-1)
  {
    return false;
  ...
  QModelIndex index = functions_model->index(f, 1, QModelIndex());
  int state=(functions_model->data(index, Qt::CheckStateRole)).toInt();
  bool checked=false;
  if(state==Qt::Checked)
  {
    checked=true;
...
void FunctionWidget::reset()
{
  ...
  for(int row = 0; row < max_functions; ++row)
  {
    ...
    QStandardItem* item=functions_model->item(row, 1);
    item->setCheckable(true);
    item->setEditable(false);
    item->setCheckState(Qt::Checked);
    index = functions_model->index(row, 2, QModelIndex());
    functions_model->setData(index, QVariant(QString("")), Qt::EditRole);
  }
}
...
{
  int scrollBarWidth=functions_tableView->verticalScrollBar()->width();
  functions_tableView->setColumnWidth(0, 150);
  functions_tableView->setColumnWidth(1, 50);
  functions_tableView->setColumnWidth(2, functions_tableView->width()-functions_tableView->verticalHeader()->width()-scrollBarWidth-table_padding-202);
}
...
void FunctionWidget::function_active_changed_slot(const QModelIndex& topLeft, const QModelIndex& bottomRight)
{
  QModelIndex index;
  for(int row=topLeft.row(); row<=bottomRight.row(); ++row)
  {
    index=functions_model->index(row, 1, QModelIndex());
    int new_state=(functions_model->data(index, Qt::CheckStateRole)).toInt();
    bool checked=false;
    if(new_state==Qt::Checked)
    {
      checked=true;
    }
    emit function_active_changed(row, checked);
  }
}

void FunctionWidget::function_names_changed_slot(const QModelIndex& topLeft, const QModelIndex& bottomRight)
{
  QModelIndex index;
  for(int row=topLeft.row(); row<=bottomRight.row(); ++row)
  {
    ...
    QString new_name=(functions_model->data(index, Qt::EditRole)).toString();
    emit function_name_changed(row, new_name.toStdString());
  }
}
...
void FunctionWidget::createGadgets()
{
  ...
  hlayout->addWidget(n_functions_SB, n_funcs, Qt::AlignRight);

  functions_tableView=new QTableView();
  functions_model=new QStandardItemModel(max_functions, 3);
  functions_model->setHeaderData(0, Qt::Horizontal, "Name");
  functions_model->setHeaderData(1, Qt::Horizontal, "Active");
  functions_model->setHeaderData(2, Qt::Horizontal, "Function");
  ...
  functions_tableView->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOn);
  ...
  connect(functions_tableView->model(), SIGNAL(dataChanged(const QModelIndex&, const QModelIndex&)), this, SLOT(function_active_changed_slot(const QModelIndex&, const QModelIndex&)));
  connect(functions_tableView->model(), SIGNAL(dataChanged(const QModelIndex&, const QModelIndex&)), this, SLOT(function_names_changed_slot(const QModelIndex&, const QModelIndex&)));
