...
parser::parser(const string& s)
{
  ...
  if( in("{", st) || in("&", st) || in("&", st) || in("!", st) || in("}", st)
                  || in("%", st) || in("?", st) || in("[", st) || in("|", st) || in(";", st) || in("`", st) || in(":", st)
  ...
  replace(st, "arctanp", ";");  replace(st, "arctan", "["); replace(st, "arccotp", "`"); replace(st, "arccot", "|"); replace(st, "tanh",   ":");
...
double acotp(double x)
{
  double acx = acot(x);
  if(acx<0)
  {
    return 3.141592653589793+acx;
  }
  else
  {
    return acx;
...
double parser::prim(bool get)
{
  ...
  switch (curr_tok)
  {
    ...
    case ARCCOT:
      return acot(prim(true));
    case ARCTANP:
      return atanp(prim(true));
    case ARCCOTP:
      return acotp(prim(true));
    case TANH:
