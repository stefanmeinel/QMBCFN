...
void MainWidget::updateSizes()
{
  ...
  for(int column=0; column<2; ++column)
  {
    if(column<1)
    {
      results_tableView->setColumnWidth(column, (results_tableView->width()-results_tableView->verticalScrollBar()->width()-table_padding)/2);
    }
    else
    {
      results_tableView->setColumnWidth(column, (results_tableView->width()-results_tableView->verticalScrollBar()->width()-table_padding)/2+(results_tableView->width()-results_tableView->verticalScrollBar()->width()-table_padding)%2-2);
    }
  }
}
...
void MainWidget::file_type_modified_slot()
{
  clear_results();
  emit file_type_modified(fileTypeCb->currentIndex());
}
...
void MainWidget::createGadgets()
{
  ...
  connect(fileTypeCb, SIGNAL(currentIndexChanged(int)), this, SLOT(file_type_modified_slot()));
  ...
  spacer1=new QSpacerItem(20,20);

  fitbutton=new QPushButton("Start fit(s)");
  ...
  results_model=new QStandardItemModel(max_parameters, 2);
